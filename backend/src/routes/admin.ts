...
router.patch('/profiles/:id', authMiddleware, adminMiddleware, async (req, res) => {
  try {
    ...
    if (typeof isStaff === 'boolean') profile.isStaff = isStaff;
...
router.post('/users/:id/roles', authMiddleware, adminMiddleware, async (req, res) => {
  try {
    ...
    if (existing) return res.status(400).json({ error: 'User already has this role' });
...
router.post('/profiles/:id/delete-request', authMiddleware, adminMiddleware, async (req: any, res) => {
  try {
...
// Execute deletion
router.delete('/profiles/:id', authMiddleware, adminMiddleware, async (req: any, res) => {
  try {
    const { reason } = req.body;
    if (!reason || reason.length < 10) return res.status(400).json({ error: 'Reason must be at least 10 characters' });
